...
namespace MyBlog.Server.Core.Logging;
// 数据库日志
public class DatabaseLoggingWriter : IDatabaseLoggingWriter, IDisposable
{
    ...
    private readonly IRepository<SysSigninLog> _sysSigninLogRepository;
    ...
    public DatabaseLoggingWriter(IServiceScopeFactory scopeFactory)
    {
        ...
        _sysOperationLogRepository =
            _serviceScope.ServiceProvider.GetRequiredService<IRepository<SysOperationLog>>();
        _sysSigninLogRepository = _serviceScope.ServiceProvider.GetRequiredService<IRepository<SysSigninLog>>();
        _sysUseRepository = _serviceScope.ServiceProvider.GetRequiredService<IRepository<SysUser>>();
    }
    ...
    public void Write(LogMessage logMsg, bool flush)
    {
            ...
            "SignIn".Equals(json.actionTypeName.ToString(), StringComparison.CurrentCultureIgnoreCase))
        {
            ...
            {
                var id = _sysUseRepository.AsQueryable().Where(x => x.Account == account).First().Id;
                if (id == 0)
                {
                    return;
                }
